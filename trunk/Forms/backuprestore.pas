...
begin
  ...
  if (SaveDialog1.Execute) then begin
    ...
    if Operation = opBackup then begin //JKOZ : does it make any diference if it is always set to false regardles of operation?
  ...
  try
    ...
    with FireBirdServices do begin
      ...
      if LowerCase(ExtractFileExt(UserFile))='.zip' then begin
        if Operation = opBackup then begin
      ...
      try
        ...
        //  StartRestore;
        case Operation of
          opBackup  :StartBackup;
          opRestore :StartRestore;
        end;

        while ServiceQuery(Res) do
          meLog.Lines.Add(Res);

        if (TempFile<>'' {using zip file}) and (Operation = opRestore) then begin
