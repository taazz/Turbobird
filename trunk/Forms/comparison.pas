...
type
  ...
  private
    FDBIndex: Integer;
    FDiffCount: Integer;
    FDBObjectsList: array [TObjectType] of TStringList; //matches turbocmmon.dbObjects
    FDBExistingObjectsList: array [TObjectType] of TStringList; //matches turbocmmon.dbObjects
    FDBRemovedObjectsList: array [TObjectType]  of TStringList; //matches turbocmmon.dbObjects
  ...
  try
    ...
    begin
      case ObjectCounter of
        ...
        ord(otViews): ObjectType:= otUDFs; //table check constraints can depend on these
        ord(otStoredProcedures): ObjectType:= otSystemTables; //for completeness
        ord(otUDFs): ObjectType:= otTables; //from here on, more or less follow regular order
      ...
      for i:= 0 to FDBObjectsList[ObjectType].Count - 1 do
      begin
        ScriptList.Clear;
        Scriptdb.ScriptTableAsCreate(FDBIndex, FDBObjectsList[ObjectType].Strings[i], ScriptList);
      ...
      begin
        ...
        FQueryWindow.meQuery.Lines.Add('create generator ' + FDBObjectsList[ObjectType].Strings[i] + ';');
        FQueryWindow.meQuery.Lines.Add('');
      end
      else
      ...
      begin
        ScriptList.Clear;
        dmSysTables.ScriptTrigger(FDBIndex, FDBObjectsList[ObjectType].Strings[i], ScriptList, True);
      ...
      for i:= 0 to FDBObjectsList[ObjectType].Count - 1 do
      begin
        fmMain.GetViewInfo(FDBIndex, FDBObjectsList[ObjectType].Strings[i], Columns, ViewBody);
        ScriptList.Text:= Trim(ViewBody);

        FQueryWindow.meQuery.Lines.Add('CREATE VIEW "' + FDBObjectsList[ObjectType].Strings[i] + '" (' + Columns + ')');
      ...
      for i:= 0 to FDBObjectsList[ObjectType].Count - 1 do
      begin
        ScriptList.Text:= fmMain.GetStoredProcBody(FDBIndex, FDBObjectsList[ObjectType].Strings[i], SPOwner);
        ScriptList.Insert(0, 'SET TERM ^ ;');
        ScriptList.Insert(1, 'CREATE Procedure ' + FDBObjectsList[ObjectType].Strings[i]);
        ScriptList.Add('^');
      ...
      if (ObjectType = otUDFs) and cxUDFs.Checked then // UDF
      for i:= 0 to FDBObjectsList[ObjectType].Count - 1 do
      begin
        ScriptList.Clear;
        ScriptList.Add('Declare External Function "' + FDBObjectsList[ObjectType].Strings[i] + '"');
        if fmMain.GetUDFInfo(FDBIndex, FDBObjectsList[ObjectType].Strings[i], ModuleName, EntryPoint, Params) then
        begin
      ...
      for i:= 0 to FDBObjectsList[ObjectType].Count - 1 do
      begin
        dmSysTables.GetDomainInfo(FDBIndex, FDBObjectsList[ObjectType].Strings[i], DomainType, DomainSize, DefaultValue, CheckConstraint, CharacterSet, Collation);

        Line:= 'Create Domain ' + FDBObjectsList[ObjectType].Strings[i] + ' as ' + DomainType;
        // String size
      ...
      begin
        ScriptList.Clear;
        FQueryWindow.meQuery.Lines.Add('create role ' + FDBObjectsList[ObjectType].Strings[i] + ';');
        FQueryWindow.meQuery.Lines.Add('');
      end
      else
      ...
      begin
        ...
        if (dmSysTables.GetExceptionInfo(FDBIndex, FDBObjectsList[ObjectType].Strings[i],
          ExceptionMesage, ExceptionDescription, ExceptionSQL, true)) then
          FQueryWindow.meQuery.Lines.Add(ExceptionSQL)
        else
          raise Exception.Create('Error scripting exception '+FDBObjectsList[ObjectType].Strings[i]);
        FQueryWindow.meQuery.Lines.Add('');
      end
      else
      ...
      for i:= 0 to FDBObjectsList[ObjectType].Count - 1 do
      begin
        Line:= FDBObjectsList[ObjectType].Strings[i];
      ...
      begin
        ...
        begin
          Line:= 'alter table ' + ATableName + ' add constraint ' + ConstraintName +
            ...
            ' (' + dmSysTables.GetConstraintForeignKeyFields(OtherFieldName, dmSysTables.MDOQuery) + ') ';
  ...
  try
    ...
    begin
      if ((ObjectType = otTables) and cxTables.Checked) or
         ...
         ((ObjectType = otUDFs) and cxUDFs.Checked) or
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            FDBObjectsList[ObjectType].Add(List[i]);
            Inc(FDiffCount);
          end
          else                                        // Exists
            FDBExistingObjectsList[ObjectType].Add(List[i]);
        end;
      end;
    end;
...
begin
  ...
  try
    ...
    begin
      ...
      Sleep(40);
      {$ENDIF}
      FDBRemovedObjectsList[ObjectType].Clear;
      ...
      begin
        ...
        begin
          ...
          FDBRemovedObjectsList[ObjectType].Add(ComparedList[i]);
          Inc(FDiffCount);
        end;
      end;
    end;
...
begin
  ...
  try
    ...
    for i:= 0 to FDBExistingObjectsList[otTables].Count - 1 do
    begin
      ...
      dmSysTables.GetTableFields(FDBIndex, FDBExistingObjectsList[otTables].Strings[i], [], FieldsList);
      dmSysTables.GetTableFields(cbComparedDatabase.ItemIndex, FDBExistingObjectsList[otTables].Strings[i], [], ComparedList);

      // Get missing fields
      for j:= 0 to FieldsList.Count - 1 do
        if ComparedList.IndexOf(FieldsList[j]) = -1 then // Add to missing list
        begin
          meLog.Lines.Add(' ' + FDBExistingObjectsList[otTables].Strings[i] + ': ' + FieldsList[j]);
          FMissingFieldsList.Add(FDBExistingObjectsList[otTables].Strings[i] + ',' + FieldsList[j]);
          Inc(FDiffCount);
        end
        else                                             // Add to existing list
          FExistFieldsList.Add(FDBExistingObjectsList[otTables].Strings[i] + ',' + FieldsList[j]);
    end;
  finally
    FieldsList.Free;
    ComparedList.Free;
...
begin
  ...
  begin
    ...
    if Exists then
      OtherFieldNames:= dmSysTables.GetConstraintForeignKeyFields(OtherFieldName, dmSysTables.MDOQuery);
    ...
    if Exists then
      COtherFieldNames:= dmSysTables.GetConstraintForeignKeyFields(COtherFieldName, dmSysTables.MDOQuery)
    else
...
begin
  ...
  for i:= 0 to FDBExistingObjectsList[otViews].Count - 1 do
  begin
    // Check for cancel button press
    Application.ProcessMessages;
    if FCanceled then
    ...
    ViewName:= FDBExistingObjectsList[otViews][i];
...
begin
  ...
  for i:= 0 to FDBExistingObjectsList[otTriggers].Count - 1 do
  begin
    // Check for cancel button press
    Application.ProcessMessages;
    if FCanceled then
    ...
    TriggerName:= FDBExistingObjectsList[otTriggers][i];
...
begin
  ...
  for i:= 0 to FDBExistingObjectsList[otStoredProcedures].Count - 1 do
  begin
    // Check for cancel button press
    Application.ProcessMessages;
    if FCanceled then
    ...
    ProcName:= FDBExistingObjectsList[otStoredProcedures][i];
...
begin
  ...
  for i:= 0 to FDBExistingObjectsList[otUDFs].Count - 1 do
  begin
    // Check for cancel button press
    Application.ProcessMessages;
    if FCanceled then
    ...
    FunctionName:= FDBExistingObjectsList[otUDFs][i];
...
begin
  ...
  for i:= 0 to FDBExistingObjectsList[otDomains].Count - 1 do
  begin
    ...
    DomainName:= FDBExistingObjectsList[otDomains][i];
...
begin
  ...
  try
    ...
    FDBRemovedObjectsList[otIndexes].Clear;
    ...
    begin
      ...
      begin
        FDBRemovedObjectsList[otIndexes].Add(CTablesList[i] + ',' + ComparedList[i]);
...
begin
  ...
  try
    ...
    FDBRemovedObjectsList[otConstraints].Clear;
    ...
    begin
      ...
      if (Po = -1) or (TablesList[Po] <> CTablesList[i]) then
      begin
        FDBRemovedObjectsList[otConstraints].Add(CTablesList[i] + ',' + ComparedList[i]);
        meLog.Lines.Add(' ' + CTableslist[i] + ':' + ComparedList[i]);
        Inc(FDiffCount);
      end;
    end;
  ...
  try
    ...
    begin
      ...
      for j:= 0 to ComparedList.Count - 1 do
        ...
        begin
          meLog.Lines.Add(' ' + FDBExistingObjectsList[otTables].Strings[i] + ': ' + ComparedList[j]);
          FRemovedFieldsList.Add(FDBExistingObjectsList[otTables].Strings[i] + ',' + ComparedList[j]);
          Inc(FDiffCount);
        end;
    end;
  finally
...
begin
  ...
  if FDBObjectsList[otIndexes].Count > 0 then
  begin
    FQueryWindow.meQuery.Lines.Add('');
  ...
  FieldsList:= TStringList.Create;
  try
    for i:= 0 to FDBObjectsList[otIndexes].Count - 1 do
    begin
      Line:= FDBObjectsList[otIndexes][i];
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if FDBRemovedObjectsList[ObjectType].Count > 0 then
        begin
          FQueryWindow.meQuery.Lines.Add('');
          FQueryWindow.meQuery.Lines.Add('-- Removed Tables');
        end;
        for i:= 0 to FDBRemovedObjectsList[ObjectType].Count - 1 do
        begin
          ObjName:= FDBRemovedObjectsList[ObjectType][i];
      ...
      if (ObjectType = otUDFs) and cxUDFs.Checked then
      begin
        if FDBRemovedObjectsList[ObjectType].Count > 0 then
        begin
          FQueryWindow.meQuery.Lines.Add('');
...
begin
  ...
  try
    ...
    FDBObjectsList[otIndexes].Clear;
        ...
        begin
          for j:= 0 to IndexList.Count - 1 do
            ...
            begin
              ...
              FDBObjectsList[otIndexes].Add(TablesList[i] + ',' + IndexList[j]);
              Inc(FDiffCount);
            end
            else
            begin
...
begin
  ...
  try
    ...
    try
      ...
      FDBObjectsList[otConstraints].Clear;
      for i:= 0 to TablesList.Count - 1 do
      begin
        ...
        dmSysTables.Init(FDBIndex);
        List.Clear;
        dmSysTables.GetTableConstraints(TablesList[i], dmSysTables.MDOQuery, List);
        dmSysTables.MDOQuery.Close;

        dmSysTables.Init(cbComparedDatabase.ItemIndex);
        ComparedList.Clear;

        if dmSysTables.GetTableConstraints(TablesList[i], dmSysTables.MDOQuery, ComparedList) then
        begin
          dmSysTables.MDOQuery.Close;
          for j:= 0 to List.Count - 1 do
            ...
            begin
              ...
              FDBObjectsList[otConstraints].Add(TablesList[i] + ',' + List[j]);
              Inc(FDiffCount);
            end
            else
