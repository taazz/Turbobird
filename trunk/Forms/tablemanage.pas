...
type
  ...
  TfmTableManage = class(TForm)
    ...
    SQLQuery1_OLD :TSQLQuery;
    SQLQuery2_OLD :TSQLQuery;
  ...
  private
    FDBIndex   :Integer;
    FTableName :string;
    FConnection:TMDODatabase; //TIBConnection;
    FSQLTrans  :TMDOTransaction; //TSQLTransaction;
  public
...
begin
  ...
  dmSysTables.GetConstraintsOfTable(FTableName, qryMain);
  sgReferences.RowCount:= 1;

  qryMain.First;
  with qryMain, sgReferences do
  while not EOF do
  begin
    RowCount:= RowCount + 1;
...
begin
  try
    ...
    with fmMain do
    begin
      FConnection:= RegisteredDatabases[dbIndex].Conn;
      FConnection.Close;
      FSQLTrans:= RegisteredDatabases[dbIndex].Trans;
      FConnection.Transaction:= FSQLTrans;
    end;
    qryMain.Close;
    qryMain.DataBase:= FConnection;
    qrySecondary.Close;
    qrySecondary.DataBase:= FConnection;
  except
    on E: Exception do
    begin
...
begin
  ...
  // qryMain should have been filled by GetTableConstraints
  while not qryMain.EOF do
  begin
    RowCount:= RowCount + 1;
    Cells[0, RowCount - 1]:= qryMain.FieldByName('ConstName').AsString;
    Cells[1, RowCount - 1]:= qryMain.FieldByName('KeyName').AsString;
    Cells[2, RowCount - 1]:= qryMain.FieldByName('OtherFieldName').AsString;
    Cells[3, RowCount - 1]:= qryMain.FieldByName('CurrentTableName').AsString;
    Cells[4, RowCount - 1]:= dmSysTables.GetConstraintForeignKeyFields(qryMain.FieldByName('CurrentFieldName').AsString, qrySecondary);

    Cells[5, RowCount - 1]:= qryMain.FieldByName('UpdateRule').AsString;
    Cells[6, RowCount - 1]:= qryMain.FieldByName('DeleteRule').AsString;
    qryMain.Next;
  end;
  qryMain.Close;
end;
...
begin
  QryMain.Close;
  QryMain.SQL.Text:= Format('SELECT RDB$Trigger_Name, RDB$Trigger_Inactive FROM RDB$TRIGGERS WHERE RDB$SYSTEM_FLAG=0 ' +
    'and RDB$Relation_Name = ''%s'' ',[FTableName]);
  QryMain.Open;
  sgTriggers.RowCount:= 1;
  with sgTriggers, QryMain do
  while not EOF do
  begin
    RowCount:= RowCount + 1;
    Cells[0, RowCount - 1]:= QryMain.Fields[0].AsString;
    if QryMain.Fields[1].AsString = '1' then
